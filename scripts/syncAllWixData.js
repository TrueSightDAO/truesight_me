...
async function syncAgroverseShipments() {
  console.log("📦 Fetching AgroverseShipments...");
  ...
  console.log(`   ✅ Wrote ${shipments.length} shipments`);
  return shipments;
}
...
async function syncStatistics() {
  ...
  try {
    ...
    console.log(`   ✅ Wrote ${stats.length} statistics items`);
    return stats;
  } catch (err) {
    console.warn(`   ⚠️  Failed to query Statistics collection: ${err.message}`);
    return [];
  }
}
...
async function syncEdgarModules() {
  console.log("🤖 Fetching EdgarModules...");
  try {
    ...
    console.log(`   ✅ Wrote ${modules.length} Edgar modules`);
    return modules;
  } catch (err) {
    console.warn(`   ⚠️  Failed to query EdgarModules collection: ${err.message}`);
    return [];
  }
}
 ...
 */
async function main() {
  console.log("🔄 Syncing all Wix CMS collections...\n");
  ...
  try {
    ...
    console.error(`   ❌ AgroverseShipments sync failed: ${err.message}`);
  }

  try {
    results.collections.exchangeRates = await syncExchangeRates();
  } catch (err) {
    console.error(`   ❌ ExchangeRate sync failed: ${err.message}`);
  }

  try {
    results.collections.statistics = await syncStatistics();
  } catch (err) {
    console.error(`   ❌ Statistics sync failed: ${err.message}`);
  }

  try {
    results.collections.edgarModules = await syncEdgarModules();
  } catch (err) {
    console.error(`   ❌ EdgarModules sync failed: ${err.message}`);
  }
